		...
		if(mechanicRepository.existsById(mechanicId))
		{
			...
			if(averageRating <= 2)
			{
				if(!mechanic.isAllowed())
					return new ResponseEntity<String>("Mechanic Already Blocked", HttpStatus.BAD_REQUEST);
				mechanic.setAllowed(false);
				mechanicRepository.save(mechanic);
				return new ResponseEntity<String>("Mechanic Blocked Due to Less Average Rating", HttpStatus.OK);
			}
			else
